...
**NOTE**: if you deploy LiteLLM in linux, the node: `host.docker.internal` (only supported by Docker Desktop Windows/Mac) is no available, you must registered when start the LiteLLM compose file adding `extra_hosts` argument under `litellm` service like this (review docker-compose.yml file):
